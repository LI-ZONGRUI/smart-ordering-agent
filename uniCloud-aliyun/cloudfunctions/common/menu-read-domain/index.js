...
function resolveDatabase(options) {
  if (options && options.db) return options.db
...
async function searchMenu({ query }, options) {
  const rows = await readDishes(resolveDatabase(options))
...
async function listAvailableDrinks(args, options) {
...
async function getDishDetail({ dishId }, options) {
  const rows = await readDishes(resolveDatabase(options), { _id: dishId })
