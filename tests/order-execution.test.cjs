...
function loadService(options = {}) {
  ...
  vm.runInNewContext(source + '\nthis.api={buildExpectedPreview,createConfirmedOrder}', context)
...
function loadPage(options = {}) {
  const calls = { create: 0, clear: 0, run: 0, preview: 0, execute: 0, navigation: [] }
  ...
  const context = {
    ...
    createConfirmedOrder: async (...args) => { calls.create += 1; return options.create ? options.create(...args) : { orderNo: 'OD1780000000000010203' } },
  ...
  vm.runInNewContext(script + `\nthis.state={query,orderPendingAction,orderCartSnapshot,orderProposalConfirmed,
    orderStatus,orderStatusType,submittingOrder,createdOrderNo,generateOrderPreview,confirmOrderProposal,
    submitConfirmedOrder,cancelOrderProposal,submitQuery,goToOrders}`, context)
